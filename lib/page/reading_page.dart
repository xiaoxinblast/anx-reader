...
class ReadingPageState extends ConsumerState<ReadingPage>
  ...
  static const _volumeKeyChannel =
      MethodChannel('com.anxcye.anx_reader/volume_keys');
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) {
        _requestReaderFocus();
        _volumeKeyChannel.setMethodCallHandler(_handleVolumeKeyMethodCall);
        _syncVolumeKeyTurnPageToNative();
  ...
  void dispose() {
    ...
    _sessionStart = null;
    audioHandler.stop();
    // Reset volume key interception on the native side when leaving the reader
    _volumeKeyChannel.invokeMethod(
        'setVolumeKeyTurnPage', {'enabled': false});
  ...
  Future<void> _handleVolumeKeyMethodCall(MethodCall call) async {
    if (!Prefs().volumeKeyTurnPage || !_readerFocusNode.hasFocus) return;
  ...
  void _syncVolumeKeyTurnPageToNative() {
    _volumeKeyChannel.invokeMethod(
        'setVolumeKeyTurnPage', {'enabled': Prefs().volumeKeyTurnPage});
  }
